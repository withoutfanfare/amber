    ...
    <table class="w-full text-sm">
      <thead>
        <tr class="border-b border-border-subtle text-left text-xs text-text-tertiary uppercase tracking-wide">
          ...
          <th class="sortable-header px-3 py-2.5 font-semibold text-right" @click="toggleSort('sizeBytes')">
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-3 py-3 text-text-secondary max-w-[200px] truncate">
            {{ snapshot.note || '—' }}
          </td>
          <td class="px-3 py-3 text-text-secondary">
            {{ getProfileName(snapshot.profileId) }}
          </td>
          <td class="px-3 py-3 text-text-secondary whitespace-nowrap">
            {{ formatDate(snapshot.createdAt) }}
          </td>
          <td class="px-3 py-3 text-text-secondary text-right font-mono whitespace-nowrap">
          ...
          <td class="px-3 py-3 text-right">
            <div class="flex items-center justify-end gap-1">
              ...
              <Button variant="ghost" size="sm" @click="emit('reveal', snapshot)" title="Show in Finder">
                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
                </svg>
              </Button>
              <Button variant="ghost" size="sm" class="text-danger hover:text-danger" @click="emit('delete', snapshot.id)">
                Delete
              </Button>
            </div>
          </td>

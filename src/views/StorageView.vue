<script setup lang="ts">
  ...
  async function handlePrune() {
    ...
    try {
      ...
      toast.success(
        `Pruned ${oldSnapshots.length} snapshot${oldSnapshots.length !== 1 ? "s" : ""}.`,
      );
      await loadStorage();
    } catch (e) {
    ...
    <div v-else class="space-y-6">
      ...
      <SCard v-if="storageInfo">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs tracking-wide text-text-secondary uppercase">Total Storage Used</p>
            <p class="text-2xl font-semibold text-text-primary">
              {{ formatSize(storageInfo.totalBytes) }}
            </p>
          </div>
          <div class="text-right">
            <p class="text-xs tracking-wide text-text-secondary uppercase">Snapshots</p>
            <p class="text-2xl font-semibold text-text-primary">
      ...
      <!-- Pruning controls -->
      <SCard>
        <h3 class="mb-4 text-sm font-semibold text-text-primary">Prune Old Snapshots</h3>
        <div class="flex items-end gap-3">
          <div class="flex items-center gap-2">
            ...
            >
            <div class="w-20">
              <SInput v-model="pruneDays" type="number" />
            </div>
            <span class="text-sm text-text-secondary">days</span>
          </div>
          <SButton variant="danger" size="sm" :loading="pruning" @click="handlePrune">
            Prune
          </SButton>
        </div>
      </SCard>

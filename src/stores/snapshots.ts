...
import type { Snapshot, SnapshotCreatePayload, SnapshotProgress, SnapshotRestoreOptions, RestoreRecord } from '@/types'
...
export const useSnapshotStore = defineStore('snapshots', () => {
  ...
  const totalSizeBytes = computed(() =>
    snapshots.value.reduce((sum, s) => sum + s.sizeBytes, 0)
  )
      ...
      onProgress.onmessage = (msg) => {
        switch (msg.event) {
          case 'started':
            ...
            break
          case 'phase':
            progress.value = { phase: msg.data.phase, message: msg.data.message, percentage: progress.value?.percentage ?? 0 }
            break
          case 'progress':
  ...
  async function restore(id: string, options?: SnapshotRestoreOptions): Promise<void> {
    ...
    try {
      ...
      const record = await invoke<RestoreRecord>('snapshot_restore', { snapshotId: id, options: options ?? null, onProgress })
  ...
  return {
    snapshots, loading, creating, restoring, error, progress,
